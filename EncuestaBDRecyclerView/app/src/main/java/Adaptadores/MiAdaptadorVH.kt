...
class MiAdaptadorVH: ArrayAdapter<Encuestado> {
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
        ...
        if (holder.txtSO?.text == "Linux") {
            with(holder.imagen) {
                holder.imagen?.setImageResource(com.example.encuestabdrecyclerview.R.drawable.linux)
            }
        }
        if (holder.txtSO?.text == "Windows") {
            with(holder.imagen) {
                holder.imagen?.setImageResource(com.example.encuestabdrecyclerview.R.drawable.windows)
            }
        }
        if (holder.txtSO?.text == "Mac") {
            with(holder.imagen) {
                holder.imagen?.setImageResource(com.example.encuestabdrecyclerview.R.drawable.mac)
            }
        }
